...
def main():
    ...
    for i in range(args.from_id, args.to_id):
        if not os.path.isfile(videos_path + data.loc[i, "id"] + ".mp4"):
            ...
            url_command = "yt-dlp -f best --get-url https://www.youtube.com/watch?v=" + str(data.loc[i, "id"])
            res1 = subprocess.run(url_command, stdout=subprocess.PIPE, shell=True).stdout.decode("utf-8").rstrip()
            if res1 == "":
                if args.verbose:
                    print("----------------------Video not available---------------------")
                continue

            download = "ffmpeg" + " -nostats -loglevel 0 -ss " + str(data.loc[i, "start"]) + " -i \"" + res1 + "\"" + \
                       " -t " + str(min(args.duration, float(data.loc[i, "end"]) - float(data.loc[i, "start"]))) + \
                       " -c:v copy -c:a copy " + videos_path + str(data.loc[i, "id"]) + ".mp4"
    ...
    if args.low_memory == "yes":
        delete_audios = "rm {0}*".format(audios_path)
        delete_videos = "rm {0}*".format(videos_path)
